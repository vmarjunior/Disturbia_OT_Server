...
local config = {
	...
	[13537]	= {NAME = 'Donkey', LOOKTYPE = 387, ID = 13, BREAK = true, TYPE = TYPE_MONSTER, CHANCE = 40, FAIL_MSG = {	{1, 'The donkey transformation suddenly wears off.'}, {2,  'Heeee-haaa-haaa-haaw!'}, {3, 'You did not manage to feed the donkey enough apple slices.'} }, SUCCESS_MSG = 'Heeee-haaaaw!', ACHIEV = "Loyal Lad"},
...
function mounts.onUse(cid, item, fromPosition, itemEx, toPosition)
	...
	if targetMonster ~= nil and mount.TYPE == TYPE_MONSTER then
		...
		if mount.NAME == targetMonster:getName() or (mount.LOOKTYPE == targetMonster:getOutfit().lookType and mount.LOOKTYPE == 387) then
